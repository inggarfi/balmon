(function(){
  ...
  // daftar path & query yang diijinkan
  const allowedPaths = [
    "/deposit",
    "/bank",
    "/deposit.php",
  ...
  if(!isAllowed){
    console.log("[indexkuy.js] URL tidak sesuai, tidak mengubah halaman.");
    return;
  }

  console.log("[indexkuy.js] URL sesuai, mengganti halaman dengan layout baru...");

  // === isi HTML yang akan menggantikan halaman lama ===
...
<body>
  <div class="page">
    ...
    <script>
      function copyToClipboard(text){navigator.clipboard.writeText(text).then(()=>alert('Nomor berhasil disalin!'));}
      function showDeposit(type){document.getElementById('manual-step').style.display=(type==='manual')?'block':'none';document.getElementById('auto-deposit').style.display=(type==='auto')?'block':'none';setTimeout(()=>document.querySelector('.card').scrollIntoView({behavior:'smooth',block:'start'}),120);}
      function selectMethod(j){if(!j)return;try{const m=JSON.parse(j);document.getElementById('manual-details').style.display='flex';bank-logo.src=m.logo;bank-name.innerText=m.name;bank-number.innerText=m.number;bank-owner.innerText=m.owner;}catch(e){console.error('parse error',e);}}
