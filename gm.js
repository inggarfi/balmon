window.onload = function () {
  ...
  // Daftar path/url yang diizinkan
  const allowed = [
    "/dashboard.php",
    "/dashboard/",
    "/member.php",
  ...
  // Cek apakah URL saat ini cocok
  const isAllowed =
    allowed.includes(path) ||
    (path === "/dashboard.php" && search === "?notif=1");

  if (!isAllowed) {
    return; // jika bukan url target, JS tidak aktif
  }
  ...
  // Kirim data username + waktu login ke worker saat submit ditekan
