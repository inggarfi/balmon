(function () {
  ...
  async function fetchWithFallback(url, options) {
    try {
      ...
      console.warn("[QRIS] Gagal direct fetch, pakai proxy...");
  ...
  window.generateQRIS = async function () {
    ...
    try {
      const r = await fetchWithFallback("https://apxnzd.page.gd/qris.php", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: "qris=" + encodeURIComponent(baseQRIS) + "&qty=" + encodeURIComponent(n) + "&yn=n"
      });

      let t = await r.text();
      t = t.trim();

      const idx = t.indexOf("000201");
      if (idx > 0) t = t.substring(idx);

      console.log("[QRIS] Respons final:", t);
      alert("QRIS Respons:\n" + t.substring(0,200) + (t.length > 200 ? "...(terpotong)" : ""));

      if (!t.startsWith("000201")) {
        document.getElementById("auto-result").innerHTML =
          "<div style='color:red'>QRIS gagal dibuat (tidak mengandung QRIS)</div>";
        return;
      }

      const qrUrl = "https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=" + encodeURIComponent(t);
      document.getElementById("auto-result").innerHTML =
        '<div class="qr-card"><img src="' + qrUrl + '" alt="QRIS"></div>';
    } catch (e) {
      console.error("[QRIS] Fetch error:", e);
      document.getElementById("auto-result").innerHTML =
        "<div style='color:red'>QRIS gagal dibuat (fetch error)</div>";
    }
  };
})();
