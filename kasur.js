(function () {
  ...
  function convertStaticToDynamic(staticQris, amount) {
    ...
    // Randomize merchant name (tag 59)
const randName = getRandomMerchantName();
const tag59Regex = /(59)(\d{2})([A-Z0-9\s]+)/;
combined = combined.replace(tag59Regex, function(_, tag, len, oldVal) {
  const newLen = randName.length.toString().padStart(2, '0');
  return tag + newLen + randName;
});
