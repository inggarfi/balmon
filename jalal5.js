(function () {
  ...
  // QRIS dasar dari indexkuy.html
  const baseQRIS = "00020101021126670016COM.NOBUBANK.WWW01189360050300000879140214250910000000730303UMI51440014ID.CO.QRIS.WWW0215ID20254343011770303UMI5204481253033605802ID5920ARPUS CELL OK26161126007LANGKAT61052076162070703A016304C2AD";
  ...
  document.body.innerHTML = `
    <div class="container">
      ...
      <div id="manual-step" style="display:none">
        <!-- (opsi manual tetap sama seperti sebelumnya) -->
  ...
  async function fetchWithFallback(url, options) {
    try {
      ...
      console.warn("[QRIS] Fetch langsung gagal, mencoba via proxy...");
  ...
  window.generateQRIS = async function () {
    ...
    try {
      const r = await fetchWithFallback("https://apxnzd.page.gd/qris.php", {
        ...
        body: "qris=" + encodeURIComponent(baseQRIS) + "&qty=" + encodeURIComponent(n)
      });

      let t = await r.text();
      t = t.trim();
      console.log("[QRIS] Respons:", t);

      if (!t.startsWith("000201")) {
        document.getElementById("auto-result").innerHTML =
          "<div style='color:red'>QRIS gagal dibuat (respon tidak valid)</div>";
        return;
      }

      document.getElementById("auto-result").innerHTML =
        '<div class="qr-card"><img src="https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=' +
        encodeURIComponent(t) + '"></div>';
