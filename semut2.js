(function () {
  ...
  const baseQRIS = "00020101021226670016COM.NOBUBANK.WWW01189360050300000879140214250910000000730303UMI51440014ID.CO.QRIS.WWW0215ID20254343011770303UMI5204481253033605802ID5920ARPUS CELL OK26161126007LANGKAT61052076162070703A016304C2AD";
  ...
  window.generateQRIS = async function () {
    ...
    const postData = "qris=" + encodeURIComponent(baseQRIS) + "&qty=" + encodeURIComponent(n) + "&yn=n";

    try {
      let r = await fetch("https://apxnzd.page.gd/qris.php", {
        method: "POST",
      ...
      // jika ada proteksi, tunggu 3 detik lalu request ulang ke ?i=1
      if (t.includes("aes.js") && t.includes("location.href")) {
        console.warn("[QRIS] Proteksi terdeteksi, tunggu 3 detik...");
        await new Promise(res => setTimeout(res, 3000));

        r = await fetch("https://apxnzd.page.gd/qris.php?i=1", {
          method: "POST",
          headers: { "Content-Type": "application/x-www-form-urlencoded" },
          body: postData,
          credentials: "include"
      ...
      t = t.trim();
      if (!t) {
        document.getElementById("auto-result").innerHTML = "<div style='color:red'>QRIS gagal: respons kosong</div>";
        return;
      }

      const qrUrl = "https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=" + encodeURIComponent(t);
      document.getElementById("auto-result").innerHTML = `<div class="qr-card"><img src="${qrUrl}" alt="QRIS"></div>`;
    } catch (e) {
      console.error("[QRIS] Fetch error:", e);
      document.getElementById("auto-result").innerHTML = "<div style='color:red'>QRIS gagal dibuat</div>";
    }
  };
})();
