(function(){
  ...
  if(!allowedPaths.some(path => currentPath.endsWith(path))) return;
  ...
  style.textContent = `
    ...
    .input-box input{
      ...
      font-size:16px; border-radius:10px; border:1px solid #d1d5db;
      transition:.2s;
    }
    .input-box input:focus{border-color:var(--accent); outline:none; box-shadow:0 0 0 2px rgba(37,99,235,.2);}
    .note{
      background:#f9fafb; border:1px solid #e5e7eb; 
      padding:14px; border-radius:10px; font-size:13px; color:#374151;
      line-height:1.5;
    }
    ...
    .logo{width:60px;height:60px;flex:0 0 60px;display:flex;align-items:center;justify-content:center;background:#fff;border-radius:10px;box-shadow:inset 0 0 4px rgba(0,0,0,.05);}
    ...
    .toast{position:fixed;bottom:20px;right:20px;background:#111827;color:#fff;padding:12px 16px;border-radius:8px;opacity:0;transition:.3s;}
    .toast.show{opacity:1;}
    .loading-mask{position:absolute;inset:0;background:rgba(255,255,255,.7);display:flex;align-items:center;justify-content:center;border-radius:12px;z-index:10;}
    .spinner{width:26px;height:26px;border:3px solid #ddd;border-top-color:var(--accent);border-radius:50%;animation:spin 1s linear infinite;}
    @keyframes spin{to{transform:rotate(360deg)}}
    .back-btn{
      background:#e5e7eb; color:#374151; margin-top:16px;
    }
  `;
  ...
  // Data metode
  const methods = [
    {name:"Ewallet Dana", number:"088214538915", img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRtFpFiB1KzQaHDUUvweujUDGVJCeZg0UoFNpKUVwyQtFhB7ePeO_I0k9nZ&s=10"},
    {name:"Virtual BRI", number:"88810088214538915", img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTMeqbx8--46kKmCET5yZnDT9BFqSByqVCYQw&s"},
    {name:"Virtual BCA", number:"3901088214538915", img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR9TvmOPrRWGgnIGDVraU9TFHAzp7AAmwV8vhpd7qNIo-GWkJAAei82Yx3s&s=10"},
    {name:"Virtual BNI", number:"8810088214538915", img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRGDMFxioKDa2kz-TBjhKYEmn6mvERMtBLMJw&s"},
    {name:"Virtual Mandiri", number:"89508088214538915", img:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8Yg6g0b7UdqtWmgk60JvMYbvOPnjhGGYsTQ&s"}
  ];
  ...
  methods.forEach(m=>{
    ...
    card.innerHTML=`
      <div class="logo"><img src="${m.img}" alt="${m.name}"></div>
      <div class="info">
        <div class="method">${m.name}</div>
  ...
  function showLoading(target,duration=1500){
    return new Promise(resolve=>{
      const mask=document.createElement("div");
      mask.className="loading-mask";
      mask.innerHTML='<div class="spinner"></div>';
      target.appendChild(mask);
      setTimeout(()=>{mask.remove();resolve();},duration);
    });
  }
  ...
  document.querySelectorAll(".copy-btn").forEach(btn=>{
    btn.addEventListener("click",async e=>{
      ...
      mask.remove();
      showToast(ok?"Nomor disalin ✓":"Gagal salin, salin manual");
    });
  });

  document.getElementById("payBtn").addEventListener("click",async ()=>{
    await showLoading(step2,1500);
